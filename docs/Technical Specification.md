...
| 작성일 | 2026-09-08 |
| 관련 문서 | [architecture.md](./architecture.md), [Feature implementation.md](./Feature%20implementation.md) |

이 문서는 **무엇을 어떤 조건으로 제공하는지**를 적는다. 객체 연결과 규칙 점검은 `architecture.md`, 기능별 발표 요약은 `Feature implementation.md`를 본다.
...
- 자유게시판, QnA, 자료실을 제공한다.
- 사이트 접속은 열어 두고, 비밀값은 `.env`에 두며, API 남용·반복 요청만 제한한다.
...
브라우저
  → Spring Boot (8282)
       ...
       └ REST API  (/api/**)
            → Application → Domain
            ← Infrastructure (Oracle, Redis, SMTP, Solapi, OAuth, 국세청)
```
...
| 회원 DB | Spring Data JPA + Oracle | `ojdbc17`, XEPDB1 |
| 게시판 DB | MyBatis 4.0.1 | 동일 Oracle |
| 인증 코드 | Redis 또는 메모리 | `PHONE_VERIFY_STORE` |
...
| Infrastructure | `com.edu.springboot.infrastructure` | JPA, MyBatis, Redis, Mail, SMS, OAuth, 보안 필터 |
...
| 기능 | 사양 |
|---|---|
| 로컬 로그인 | 아이디(이메일) + 비밀번호. BCrypt. 세션 쿠키 (`credentials: 'include'`) |
| 로그아웃 | 세션 무효화 |
...
| 게시판 | 쓰기 | 수정·삭제 | 좋아요 | 댓글 |
|---|---|---|---|---|
| 자유 | 비회원 가능, 글 비밀번호 | 글 비밀번호 또는 작성자 | 게스트 허용(쿠키), 회원은 DB 중복 방지 | 없음 |
...
| 조회수 | 쿠키 + `ViewCountPolicy`. 하루 1회만 증가 |
...
| POST | `/api/boards/free` | 공개 | 자유 글쓰기 |
...
홈페이지와 정적 자원은 막지 않는다. 반복 호출과 비밀값 유출을 줄이는 것이 목표다. 회선 포화형 디도스는 이 한도만으로 막을 수 없고 CDN/WAF가 필요하다.
...
| 항목 | 사양 |
|---|---|
| CORS | `CORS_ALLOWED_ORIGINS`만 허용. 자격 증명 허용 |
...
| 포트 | 현재 구현 | 설정 |
|---|---|---|
| `MemberRepository` | JPA | Oracle |
| 게시판 Repository | MyBatis | Oracle |
| `PhoneVerificationStore` | Redis / 메모리 | `PHONE_VERIFY_STORE` |
...
로컬 CORS 기본값: `http://localhost:8282`, `http://127.0.0.1:8282`.
...
선행 조건: Oracle(기본 `localhost:1523/XEPDB1`), Redis(인증 코드 저장소를 redis로 둘 때), `.env` 작성.

---

## 11. 제약 · 알려진 한계

- CSRF는 꺼져 있다. 같은 출처 SPA+세션 전제다.
- 앱 레이트 리밋은 단일 인스턴스 메모리 기준이다. 다중 인스턴스에서는 IP 한도가 서버마다 따로 잡힌다.
- 인증 컨트롤러가 Infrastructure `ClientIpResolver`를 직접 주입한다. `AuthController`는 `MemberSessionBinder`를 주입한다. 상세는 `architecture.md` 위반 표.
- 이메일 인증 결과 타입 이름이 아직 `VerifyPhoneCodeResult` 등 휴대폰 쪽을 재사용한다.
...
| 문서 | 역할 |
|---|---|
| 이 문서 (`기술사양서.md`) | 현재 시스템의 기능·API·보안·환경 사양 |
| `architecture.md` | 계층·객체 연결, 규칙 준수 점검 |
| `Feature implementation.md` | 기능별 구현 발표 요약 |
